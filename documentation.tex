...
\mypara Please refer to the corpus as the `Bangor \textit{Siarad}' corpus, and provide a link to the website by which you accessed the corpus, either \textit{\url{bangortalk.org.uk}} or \textit{\url{talkbank.org}}. Please also cite:
\begin{quotation}
\noindent Deuchar, M., P. Davies, J. Herring, M. Parafita Couto, and D. Carter (2014). Building bilingual corpora. In: E. M. Thomas and I. Mennen (Eds.), \textit{Advances in the Study of Bilingualism}, pp. 93–111. Bristol: Multilingual Matters.
\end{quotation}
...
\mypara The Siarad corpus is licensed under the GNU GPL\footnote{gnu.org/licenses/gpl.html} if retrieved from \textit{bangortalk.org.uk}, and under Creative Commons BY-NC-SA\footnote{creativecommons.org/licenses/by-nc-sa/3.0} if retrieved from \textit{talkbank.org}.
...
\mypara Each main tier is divided into units which we call `words' for the purposes of these conventions. With some exceptions (see 3.1.3) a word is considered to be a continuous sequence of characters containing no spaces, as found in Geiriadur Prifysgol Cymru \citep{Thomas1967}, Geiriadur yr Academi \citep{Griffiths1995}, Cysgeir \citep{Bedwyr2008} or the Oxford English Dictionary online \citep{OED2008}. These are referred to as GPC, GyrA, Cysgeir and OED respectively throughout this document. Where items are entered as two hyphenated words in these reference dictionaries, they are connected by underscore in the transcripts. When one of the reference dictionaries offers more than one alternative (e.g. \textit{minibus}, \textit{mini-bus} or \textit{mini bus}), or when the reference dictionaries differ from each other, the most compact alternative is chosen (\textit{minibus} in this case).

\mypara Other items which are treated as words are:
\begin{enumerate}
\item interjections and interactional markers, e.g. \textit{ah}, \textit{er}, \textit{um} etc.
\item proper names (including names of books, films, organisations etc.), a sequence of words being connected by underscores, e.g. \textit{Elton_John}, H\textit{ong_Kong}, \textit{Sweet_Valley_High}
\item abbreviations (connected by underscore), e.g. \textit{N_S_P_C_C}
\item some prepositions and adverbs, usually represented as two words, whose individual parts are meaningless or difficult to translate in isolation, e.g. \textit{oddi_wrth}. See Table \ref{phrasewords} - most of these are normally translated into just a single English word.
\end{enumerate}
...
\mypara Contractions that do not have entries in one of the Welsh-language reference dictionaries (namely GPC, GyrA or Cysgeir) or in \citet{King2003}, are transcribed in full, but the unpronounced parts are bracketed. For example, the pronunciation of \textit{fel yna} `like that' as [vɛla] in speech is represented in the transcripts as \textit{fel (yn)a}. 

\mypara There are some continuous sequences of characters in the main tier which are not treated as words. These include simple events such as \textit{\&=laughs} (see CHAT\footnote{References to section numbers in the online CHAT manual are to the version dated March 4, 2014.} 7.8.1), \textit{xxx} for unintelligible material, or the use of an ampersand plus phonetic characters for intelligible sounds without clear meaning (see CHAT 6.4 for both).


\section{Language marking}

\mypara Each word in the main tier has its language source identified. The default language is identified as that providing the greatest number of words in the transcript and is Welsh in all the transcripts. Welsh words are unmarked but English words are identified with the tag \textit{@s:eng}. Words which could come from both Welsh and English are considered to be of `undetermined' language and are marked \textit{@s:cym\&eng}, where \textit{cym} represents Welsh and \textit{eng} English. An entire utterance in English, the non-default language, is marked with a precode \textit{[- eng]} instead of marking each word as English.
...
\mypara Words which contain two or more morphemes from different languages are marked as mixed-language words, e.g. \textit{concentrate_io@s:eng+cym} `to concentrate'. However, where a word containing at least one English morpheme and at least one Welsh morpheme is included in one or more of the Welsh-language reference dictionaries, it is marked as a Welsh word. For example, the English word \textit{use} forms the basis of the Welsh word \textit{iwsio} `to use' but the entire word is considered to be Welsh (and transcribed without a language marker as \textit{iwsio}) because it is included in one of the Welsh-language reference dictionaries.

\mypara The language marker \textit{@s:cym\&eng} is used with words where the language source is undetermined. It marks words that occur in the lexicon of both languages (as determined by the Welsh-language reference dictionaries for Welsh or by the OED for English), that are pronounced in a way that is possible both in Welsh and in English, e.g. [əŋkl] (\textit{uncle} in English or \textit{yncl} in Welsh) or [mat]  (\textit{mat} in both languages). \textit{@s: cym\&eng}  also marks a specified list of interjections and interactional markers, e.g. \textit{ah}, \textit{aha}, \textit{hmm}, \textit{oh}, \textit{ooh}, \textit{um}. Other interjections and interactional markers are assigned language markers according to their inclusion (or not) in the reference dictionaries. For example, \textit{ych} (a marker of disgust equivalent to `yuk' in English) is unmarked as it is considered to be Welsh and only found in the Welsh-language reference dictionaries.

\mypara Where a lexeme could belong to both languages, but its pronunciation in a specific occurrence belongs unambiguously to one language only, it will be assigned a language source and marked or not accordingly, depending on its pronunciation. For example, \textit{toast@s:eng} is used where the word is pronounced with [əʊ] as in English only, but \textit{toast@s:cym\&eng} where the word is pronounced with [o] as in Welsh or some varieties of Welsh English.

\mypara Proper names and titles are marked\textit{ @s:cym\&eng} (undetermined) unless there are alternatives in each language in general use, e.g. \textit{Elton_John@s:cym\&eng}, \textit{One_Flew_Over_the_Cuckoo's_Nest@s:cym\&eng}, \textit{Hong_Kong@s:cym\&eng}, \textit{\url{Tebot_Piws@s:cym\&eng}} (a Welsh-language pop group, literally meaning `purple teapot') but \textit{Cardiff@s:cym\&eng}, \textit{Caerdydd} (the Welsh word for `Cardiff').

\mypara According to GPC, the `-s' plural ending is an established loan in the Welsh lexicon. Any plural formed with the `-s' ending is assigned the language source of the previous morpheme. For example, \textit{pregethwrs} is unmarked like the singular form \textit{pregethwr} `preacher', but \textit{dolphins@s:cym\&eng} is marked undetermined like \textit{dolphin@s:cym\&eng} and \textit{dogs@s:eng} English like \textit{dog@s:eng}.

\mypara In multi-word phrases, each word is tagged separately, regardless of the phrase's internal syntax. For example, in \textit{traffic@s:cym\&eng lights@eng}, \textit{traffic} is coded as undetermined, although the syntax of the whole phrase is English.

\section{Orthography}

\mypara Words marked as \textit{@s:eng} (English) are transcribed in standard English orthography, including contractions, such as \textit{isn't}. Some non-standard spellings for colloquial forms such as \textit{gonna} are used.

\mypara Words whose language source is undetermined are transcribed in English rather than in Welsh orthography, e.g. \textit{acid@s:cym\&eng} rather than \textit{asid@s:cym\&eng}. This is in order to make the corpus more accessible to non-Welsh-speakers who might use the data. 

\mypara When words marked as English or undetermined are mutated (where the sound of an initial consonant is changed depending on the grammatical context, see for example \citealt[pp14--20]{King2003}, the initial (mutated) sound is written in Welsh orthography and the rest in English, e.g. \textit{ei firthday@s:eng} = `his birthday'; \textit{ei goat@s:eng} = his coat. In the case of words that begin with `qu' in English orthography but that are mutated in the data, the mutated sound and the following [w] are written in Welsh orthography, e.g. \textit{question} (unmutated), \textit{gwestion} (soft mutation), \textit{chwestion} (aspirate mutation), \textit{nghwestion} (nasal mutation).
...
\begin{enumerate}
\item We transcribe some non-standard verb-noun suffixes, e.g. \textit{-ian} in \textit{swnian} `to grumble' rather than \textit{-io} in the standard form \textit{swnio}. We also transcribe non-standard plural forms of nouns, e.g. \textit{cobenni} `night-dresses' rather than the standard form \textit{cobannau}.
\item We represent non-standard usage of inflected prepositions. Agreement markers for person and number show considerable variation in the spoken language. Thus one may, for example, find several forms for `to you' (plural/respect form), such as \textit{wrthoch chi} (the variant found in \citet{King2003}, \textit{wrthych (chi)} (more formal variant, e.g. prescribed in \citet{Thomas1996} as well as \textit{wrthach chi} (more colloquial, northern variant). The orthography used in the transcripts is based on pronunciation. 
\item Northern second person singular verb and preposition endings not usually represented in writing are transcribed with a final \textit{-a} where they are followed by the pronoun \textit{chdi}, e.g. \textit{oedda chdi} `you were', \textit{arna chdi} `on you'. Where they occur in isolation, they are transcribed as \textit{-achd}, e.g. \textit{oeddachd} `you were/weren't you', \textit{arnachd} `on you'.
\item We do not represent morpheme-final [v] when it is not pronounced. For example, [pɛntre] `village' is written \textit{pentre} in the transcripts rather than \textit{pentref} (as the word is represented in the Welsh-language reference dictionaries).
...
\item Morphemes in Welsh which are usually written with an initial apostrophe, such as the possessive pronoun \textit{'w}, and the marking of the ellipsis of a possessive pronoun in e.g. \textit{'nhad} `my father', do not have this initial apostrophe marked in the transcripts owing to the conventions of CHAT.
\item We have represented mutation (sound change to initial consonants) or its absence without following prescriptive rules as to where mutation might or might not be expected. Thus the Welsh form of `in Cardiff' may be transcribed \textit{yn Caerdydd} (with an initial [k] on the placename) and \textit{yn Gaerdydd} (with initial [g]), as well as the standard form \textit{yng Nghaerdydd} (with initial [ŋ̥], according to what is heard. We have also transcribed the aspirate mutation of /m/ and /n/ after the 3rd singular feminine possessive adjective common in northern varieties, e.g. \textit{ei mham} `her mother', with initial [m̥) , rather than standard \textit{ei mam}' with initial [m].
\end{enumerate}
...
\caption{Colloquial forms\label{colloquial}} \\
\hline
...
cordwellt & cordwellt & cordgrass & technical term listed on termau.org but not in our reference dictionaries \\ 
...
dafedd & edafedd & yarn & mutates to \textit{ddafedd} \\ 
diwc & duwcs & gosh &  \\ 
...
gewin, gwinedd & ewin, ewinedd & claw(s)/ finger nail(s) & colloquial form listed in GPC article for \textit{ewin} \\ 
gosa & oni bai & unless & heard in north-western varieties \\ 
hompen & homer & huge thing & form used by speaker from the south-west \\ 
jaman & --- & (embarrass) & \textit{cael jaman} = Caernarfon expression for 'being proved wrong'\footnote{See \url{youtube.com/watch?v=Z-x7zLAZdLM}} \\ 
molchi & ymolchi & wash oneself & mutates to \textit{folchi} \\ 
nunman & unman & nowhere & widespread \\ 
olradd & ôl-raddedig & postgraduate & heard in Welsh universities \\ 
penwsnos & penwythnos & weekend & GPC has an entry for \textit{wsnos} \\ 
perchynu & perchen & own & form used several times by 16 year-old native speaker \\ 
pwpŵo & --- & poo (verb) & \textit{pwpŵo} is listed in GPC meaning `talking derogatively' \\ 
...
mag & mae & (he/she/it) is & 3rd singular present form of \textit{bod} `to be' heard in south-western varieties \\ 
...
\hline
\noalign{\medskip}
argob & argoel & gosh & interactional marker based on \textit{arglwydd} `lord' \\ 
asu & --- & gosh & interactional marker based on \textit{Iesu} `Jesus' \\ 
diwc & duwcs & gosh & variant listed in GPC under \textit{duwcs} \\ 
duwarth & duwcs & gosh & interactional marker based on \textit{duw} `god' \\ 
duwedd & duwcs & gosh & interactional marker based on \textit{duw} `god' \\ 
ewadd & ew & wow &  \\ 
iargoel & argoel & gosh & interactional marker based on \textit{arglwydd} `lord' \\ 
iesgob & esgob & gosh & interactional marker based on \textit{Iesu} `Jesus' \\ 
myn_diân_i & --- & by heck & interactional marker based on \textit{diawl} `devil' \\ 
wannwyl & duw annwyl & good lord & a contraction of \textit{duw annwyl}  \\ 
...
\mypara Each word (see 3.1.2 and 3.1.3) in the main tier is given a manual gloss in the gloss tier (\textit{\%gls}) and an automatic gloss in a second gloss tier (\textit{\%aut}) which has been automatically generated using the Bangor Autoglosser (\url{bangortalk.org.uk/autoglosser.php}): for further details see \citet{Donnelly2011}. 

\mypara Non-words (see 3.1.5) are not glossed, with the exception of \textit{xxx}, which are represented by the same characters in the manual gloss (\textit{\%gls}), while being omitted for readability in the autogloss (\textit{\%aut}).
...
\mypara Proper names (including names of books, films, organisations etc.) marked as English or undetermined are glossed as they appear in the main tier. For example, \textit{Hong_Kong@s:cym\&eng} is glossed as `Hong_Kong', \textit{Cardiff@s:eng} is glossed as `Cardiff' and \textit{Tebot_Piws@s:cym\&eng} is glossed as `Tebot_Piws'. However, proper names marked as Welsh are glossed with their English-language equivalents. For example, \textit{Caerdydd} is glossed as `Cardiff'.

\mypara Lexical information always precedes morphological information in the gloss. A full stop (.) is used to separate morphological information from lexical information (e.g. go.NONFIN) and also to separate two pieces of morphological information (e.g. PRON.3SM). Some manual glosses contain only morphological information, such as POSS.2S for the 2nd singular possessive adjective \textit{dy}.

\mypara The underscore is used in the gloss tier to connect more than one lexical item in a gloss, where the English translation of a single Welsh word involves more than one word. For example, \textit{neithiwr} is glossed as `last_night'.
...
DET & determiner \\ 
F & feminine  \\ 
FUT & future/habitual present (verb \textit{bod} `to be' only) \\ 
IM & interactional marker/exclamation, e.g. \textit{um}, \textit{oh} \\ 
IMP & imperfect (verb \textit{bod} `to be' only) \\ 
IMPER & imperative \\ 
...
POSS & possessive \\ 
POSSD & possessed \\ 
PRES & present tense (verb \textit{bod} `to be' only) \\ 
PRON & pronoun  \\ 
PRT & particle \\ 
REL & relative \\ 
S & singular \\ 
...
\mypara Gender-specific adjectives in Welsh are not marked for gender in the gloss. For example, \textit{gwyn} (used to modify masculine nouns) and \textit{wen} (used to modify feminine nouns) are both glossed simply `white' in the manual glosses but `white.ADJ.M' and `white.ADJ.F+SM' in the automatic glosses.

\mypara Numerals are glossed for gender where appropriate. For example, \textit{dau} and \textit{dwy} are glossed as `two.M' and `two.F' respectively.  The autogloss is be `two.NUM.M' and `two.NUM.F' respectively.

\mypara Welsh collective nouns are glossed by the English plural. For example, \textit{moch} (singular collective noun indicating `pigs') has the gloss `pigs'.  The automatic gloss is  `pigs.N.M.PL'.
...
\noindent `her mother'\\
\textit{ei mam} : POSS.3S mother\\
\textit{ei mham}: POSS.3SF mother\\
\textit{ei mam hi} : POSS.3SF mother PRON.3SF\\

\noindent `his mother'\\
\textit{ei fam} : POSS.3SM mother\\
\textit{ei fam e} : POSS.3SM mother PRON.3SM\\
\textit{ei mam e} : POSS.3SM mother PRON.3SM\\

\noindent The above applies also to possessive constructions involving non-finite verbs preceded by \textit{ei}. For example:\\

\noindent `he was born'\\
\textit{gaeth (e) ei eni}: get.3S.PAST (PRON.3SM) POSS.3SM bear.NONFIN\\

\noindent `he/she was shot'\\
\textit{gaeth ei saethu}: get.3S.PAST POSS.3S shoot.NONFIN\\
...
\noindent `my father'\\
\textit{nhad} : father.POSSD.1S\\

\noindent (However, the automatic gloss will mark \textit{nhad} as `father.N.M.SG+SM'.)\\

\noindent Contrast this with the following:\\

\noindent \textit{fy nhad} : POSS.1S father\\
\textit{nhad i} : father PRON.1S\\
\textit{fy nhad i} : POSS.1S father PRON.1S\\
